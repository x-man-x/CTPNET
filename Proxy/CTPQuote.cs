...
namespace HaiFeng
{
	public class CTPQuote : Quote
	{
        ...
        private void CTPOnRtnForQuoteRsp(ref CThostFtdcForQuoteRspField pForQuoteRsp) {

            String ExchangeID = pForQuoteRsp.ExchangeID;
            String ForQuoteSysID = pForQuoteRsp.ForQuoteSysID;
            String ForQuoteTime = pForQuoteRsp.ForQuoteTime;
            String InstrumentID = pForQuoteRsp.InstrumentID;
            String TradingDay = pForQuoteRsp.TradingDay;
            

            QuoteData quoteData = new QuoteData();


            if (_OnRtnQuote == null) return;
            _OnRtnQuote(this, new QuotedataEventArgs
            {
                Value = quoteData
            });
        ...
        public override int ReqSubscribeForQuoteRsp(params string[] pInstrument)
        {
            int size = Marshal.SizeOf(typeof(IntPtr));
            IntPtr insts = Marshal.AllocHGlobal(size * pInstrument.Length);
            var tmp = insts;
            ...
            return (int)_q.SubscribeForQuoteRsp(insts, pInstrument.Length);
        }
